...
const LANGUAGES = ['ro', 'ru', 'sq', 'bg', 'en'];
...
module.exports = (subscriber) => {
	...
	if (!utils.isemail(subscriber.email)) {
		logger.error('No email: ' + subscriber.email);
		return Data.EmailSubscriber.remove({
				where: {
					_id: subscriber.id
				}
			})
			.then(function() {
				logger.warn('removed subscriber:', subscriber);
			});
		// return Promise.resolve();
	}
				...
				return getForecast(place)
					.then((forecast) => {
						...
						return processEmail(subscriber, place, forecast)
							.then(() => {
								logger.info('Email sent: ' + subscriber.email);
